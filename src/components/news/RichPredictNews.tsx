    ...
    return (
        <div className="sportName soccer overflow-hidden rounded-sm border border-white/5 shadow-2xl my-4 relative group/news">
            ...
            <div className="headerLeague__wrapper bg-gradient-to-b from-[#164e63] to-[#083344] border-t border-white/20 border-b border-black/40 shadow-lg relative z-10">
                <div className="wcl-header_HrElx py-2 px-3 flex items-center justify-between">
                    <div className="flex items-center min-w-0">
                        {/* 1:1 Official Logo from Header.tsx */}
                        ...
                        <span className="text-base md:text-xl font-black tracking-tighter text-white uppercase font-[Klapt] leading-none ml-[-6px] md:ml-[-10px] truncate">
                            RICH<span className="text-[var(--fs-yellow)]">PREDICT</span> <span className="font-sans text-[8px] md:text-[10px] text-white/40 ml-1 tracking-[0.2em] font-black italic">NEWS</span>
                        </span>
                    </div>

                    <div className="flex items-center gap-2 shrink-0">
                        <div className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 bg-black/20 rounded-sm border border-white/5">
                            <span className="animate-pulse w-1 h-1 bg-[var(--fs-yellow)] rounded-full" />
                            <span className="text-[8px] font-black text-white/50 uppercase tracking-widest">LIVE_FEED</span>
                        </div>
            ...
            <div className="relative bg-[var(--fs-header)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/10 p-4 gap-6 md:gap-0">
                    {posts.slice(0, 3).map((post) => (
                        ...
                        >
                            {/* Card Image with Internal Overlay */}
                            <div className="relative aspect-[16/9] overflow-hidden rounded-md bg-white/5 border border-white/10 shadow-lg mb-3">
                                {post.image_url ? (
                                    <img
                                        src={post.image_url}
                                        alt={post.title}
                                ...
                                {/* Bottom Inner Overlay for Text contrast */}
                                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-80" />

                                {/* Featured Tag style overlay */}
                                <div className="absolute top-2 right-2 px-2 py-0.5 bg-[var(--fs-yellow)] text-black text-[7px] font-black uppercase tracking-tighter rounded-sm shadow-xl">
                                    TRENDING
                                </div>
                            </div>

                            {/* Title with brand-specific line height and color */}
                            <h3 className="text-[12px] md:text-[13px] font-bold text-white/95 leading-tight group-hover:text-[var(--fs-yellow)] transition-all duration-300 line-clamp-2 md:h-10">
                                {post.title}
                            </h3>

                            {/* Subtle metadata overlay */}
                            <div className="flex items-center gap-2 mt-2 opacity-40 group-hover:opacity-100 transition-opacity">
                                <span className="w-4 h-[1px] bg-white/40 group-hover:bg-[var(--fs-yellow)] transition-colors" />
                ...
                <div className="p-4 border-t border-white/5 bg-black/20 flex justify-center mt-2">
                    <Link
                        href="/news"
                        className="group/btn px-8 py-2 text-[9px] font-black uppercase bg-gradient-to-r from-white/5 to-white/[0.02] border border-white/10 rounded-md text-white/60 hover:text-white hover:border-[var(--fs-yellow)] transition-all tracking-[0.3em] flex items-center gap-3"
                    >
                        GO TO ALL NEWS
