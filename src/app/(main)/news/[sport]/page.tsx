...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const url = `https://richpredict.com/news/${sport.toLowerCase()}`;
...
export default async function SportNewsPage({ params }: Props) {
    const { sport } = await params;
    const sportSlug = sport.toLowerCase();
    const capitalizedSport = sport.toUpperCase();

    // Fetch news for specific sport and predictions for right sidebar
    ...
    const predictions = await getPredictions(10, sportSlug === 'football' ? 'Football' : 'Tennis');
        ...
        <div className="flex flex-col min-h-screen bg-[#00141e] text-white font-sans">
            ...
            <div className="bg-[#0b1f27] border-b border-white/5 py-3 overflow-x-auto scrollbar-hide">
                <div className="max-w-[1240px] mx-auto w-full px-2 flex items-center gap-6 min-w-max">
                    ...
                    {['FOOTBALL', 'TENNIS', 'BASKETBALL', 'HOCKEY', 'GOLF'].map((cat) => (
                        <Link
                            key={cat}
                            href={`/news/${cat.toLowerCase()}`}
                            className={cn(
                                "text-[11px] font-bold uppercase tracking-tight transition-all pb-1 border-b-2",
                                cat.toLowerCase() === sportSlug ? "text-white border-red-500" : "text-white/60 border-transparent hover:text-white"
                            )}
                        >
                            {cat}
                        </Link>
                ...
                <div className="space-y-8">
                    <section className="bg-[#051a24] rounded-xl overflow-hidden shadow-2xl p-6">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h1 className="text-xl md:text-3xl font-black uppercase tracking-tighter text-white/90 font-outfit">
                        ...
                        {topNews ? (
                            ...
                            <div className="h-64 flex items-center justify-center italic text-white/20 uppercase tracking-widest text-[10px]">No {sportSlug} news available at the moment.</div>
                        )}
                                    ...
                                    <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-black border border-white/5">
                                        ...
                                        <div className="absolute bottom-0 left-0 p-3 w-full bg-gradient-to-t from-black/80 to-transparent">
                                            <div className="text-[8px] font-black text-white/60 uppercase tracking-widest flex flex-col font-sans">
                                                <span className="text-red-500 leading-none mb-1">{capitalizedSport}</span>
                                ...
                                <Link key={item.id} href={`/news/${item.slug}`} className="flex gap-4 group cursor-pointer bg-[#051a24] p-3 rounded-xl border border-white/5 hover:bg-white/[0.04] transition-all">
                                    ...
                                    <div className="flex flex-col justify-center space-y-1">
                                        ...
                                        <div className="flex items-center gap-2 text-[10px] text-white/30 font-bold uppercase tracking-widest font-sans">
                                            <span className="text-red-500/60">{capitalizedSport}</span>
                                            <span>•</span>
                ...
                <aside className="hidden lg:flex flex-col space-y-6">
                    <div className="bg-[#001e28] border border-[var(--fs-yellow)]/20 rounded-xl p-6 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 flex flex-col items-center text-center space-y-4">
                            ...
                            <button className="w-full bg-[var(--fs-yellow)] text-black py-3 rounded-lg text-[11px] font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 font-outfit underline-none">
