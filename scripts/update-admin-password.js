...
async function updatePassword() {
    console.log('Updating password for info@richpredict.com...');
    ...
    if (existing) {
        console.log('Found user, updating password to Samir_1155!...');
        const { error: updateError } = await supabase
            .from('admins')
            .update({ password: 'Samir_1155!' })
            .eq('email', 'info@richpredict.com');

        if (updateError) console.error('Error updating password:', updateError);
        else console.log('Successfully updated password for info@richpredict.com');
    } else {
        console.log('User not found, creating with password Samir_1155!...');
